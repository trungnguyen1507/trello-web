...
export const createNewBoardAPI = async (data) => {
  const response = await axiosInstance.post(`${API_ROOT}/v1/boards`, data)
  toast.success('Board created successfully!')
  return response.data
}

export const updateCardDetailAPI = async (cardId, updateData) => {
  const response = await axiosInstance.put(`${API_ROOT}/v1/cards/${cardId}`, updateData)
  return response.data
}

export const inviteUserToBoardAPI = async (data) => {
  const response = await axiosInstance.post(`${API_ROOT}/v1/invitations/board`, data)
